...
def create_materials():
    ...
    hulls = [ "hull1.jpg","hull2.jpg","hull3.jpg","foss_texture.png","foss_texture2.png","foss_texture3.png","foss_texture4.png","foss_texture5.png" ]
    hull = choice(hulls)
    #print("spaceship hull: " + hull)
    #hull = "foss_texture4.png"

    # Load up the hull normal map
    hull_normal_colortex = create_texture(
        'ColorTex', 'IMAGE', resource_path('textures', hull))
        #'ColorTex', 'IMAGE', resource_path('textures', 'hull_normal.png'))
    hull_normal_colortex.use_normal_map = False
    ...
    mtex.texture = create_texture(
        'ColorTex', 'IMAGE', resource_path('textures', 'hull_lights_diffuse2.png'))
    ...
    mtex = mat.texture_slots.add()
    mtex.texture = create_texture(
        'ColorTex', 'IMAGE', resource_path('textures', 'hull_lights_emit2.png'), False)
    mtex.texture_coords = 'GLOBAL'
    mtex.mapping = 'CUBE'
    mtex.use_map_emit = True
    mtex.emit_factor = 5.0
    mtex.blend_type = 'ADD'
    ...
    # Add world texture
    space_list = [ "space1.jpg","space2.jpg","space3.jpg","space4.jpg","space5.jpg","space6.jpg","space7.jpg","space8.jpg" ]
    space = choice(space_list)
    print("Using background: " + space)
    wtex = bpy.data.textures.new('texture', type = 'IMAGE')
    wtex.image = bpy.data.images.load(basedir + 'textures/' + space)
    www = bpy.data.worlds[0].texture_slots.add()
    www.texture = wtex
    #www.texture_coords = 'ANGMAP'
    www.use_map_horizon = 1
    www.use_map_blend = 0
    ...
    light_obj.location = (6.0,6.0,6.0)
...
if __name__ == "__main__":
	...
	# remove the Cube
        ...
        camera_pole_pitch_max = 40 # degrees
	...
	# --- tweaked this to do a very shirt movie of a single ship -steve
        ...
        yaw_rate = 110 # degrees/sec
        yaw_offset = 250 # degrees/sec
        camera_pole_rate = 1
        camera_pole_pitch_min = 20 # degrees
        camera_pole_pitch_max = 25 # degrees
        camera_pole_pitch_offset = 0 # degrees
        camera_pole_length = 10
        camera_refocus_object_every_frame = True
        fov = 70 # degrees
        fps = 30
        res_x = 540
        res_y = 405
        ...
        while movie_duration < total_movie_duration:
            ...
            script_path = bpy.context.space_data.text.filepath if bpy.context.space_data else __file__
            folder = output_path if output_path else os.path.split(os.path.realpath(script_path))[0]
            filename = os.path.join('renders', 'animated', timestamp + '_' + str(frame).zfill(5) + '.png')
            bpy.data.scenes['Scene'].render.filepath = os.path.join(folder, filename)
